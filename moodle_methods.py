...
def log_in(username, password):
    if driver.current_url == locators.moodle_url:   # check we are on the home page
        ...
        if driver.current_url == locators.moodle_login_page_url:   # check we are on the login page
            ...
            if driver.title == locators.moodle_dashboard_page_title and \
                    driver.current_url == locators.moodle_dashboard_url:
...
def create_new_user():
    ...
    Select(driver.find_element(By.ID, 'id_maildisplay')).\
        select_by_visible_text('Allow everyone to see my email address')
    sleep(0.25)
...
def delete_user():
    ...
    print('\' browser list of user page\' is displayed.')
    sleep(0.25)
    # Check we can search user by email.
    print(f'-----search for user by emailaddress: {locators.email}')
    sleep(0.25)

    search_user()

    driver.find_element(By.XPATH, '//a[contains(@href, "delete=")]').click()
    sleep(0.25)
